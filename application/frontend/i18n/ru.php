...
return array(
    ...
    'blog'          => array(
        ...
        'add'                  => array(
            ...
            'fields' => array(
                ...
                'type'        => array(
                    'label'       => 'Тип блога',
                    'note_open'   => 'Открытый — к этому блогу может присоединиться любой желающий, заметки видны всем',
                    'note_close'  => 'Закрытый — присоединиться можно только по приглашению администрации блога, заметки видят только подписчики',
                    'value_open'  => 'Открытый',
                ...
                'skip_index'  => array(
                    'label' => 'Не выводить заметки на главную',
                    'note'  => 'Все заметки из этого блога не смогут попадать на главную страницу',
                ),
            ),
            // Сообщения
            'alerts' => array(
        ...
        'remove'               => array(
            ...
            'remove_topics' => 'Удалить заметки',
            'move_to'       => 'Переместить заметки в блог',
            // Сообщения
            'alerts'        => array(
                'success'             => 'Блог успешно удален',
                'not_empty'           => 'Вы не можете удалить блок с записями. Предварительно удалите из блога все записи.',
                'move_error'          => 'Не удалось переместить заметки из удаляемого блога',
                'move_personal_error' => 'Нельзя перемещать заметки в персональный блог', // TODO: Remove?
            )
        ),
        /**
         * Управление блогом
    ...
    'emails'        => array(
        ...
        'comment_new'           => array(
            'subject'     => 'Новый комментарий к заметке',
            'text'        =>
                'Пользователь <a href="%%user_url%%">%%user_name%%</a>
				оставил новый комментарий к заметке <b>%%topic_name%%</b>,
				прочитать его можно перейдя по <a href="%%comment_url%%">этой ссылке</a>
				<br><br>
				%%comment_text%%
				%%unsubscribe%%',
            'unsubscribe' => '<a href="%%unsubscribe_url%%">Отписаться от новых комментариев к этой заметке</a>'
        ),
        ...
        'topic_new'             => array(
            'subject' => 'Новая заметка в блоге',
            'text'    =>
                'Пользователь <a href="%%user_url%%">%%user_name%%</a>
				опубликовал в блоге <b>%%blog_name%%</b>,
				новая заметка &mdash; <a href="%%topic_url%%">%%topic_name%%</a>'
        ),
        // Смена почты
    ...
    'activity'      => array(
        ...
        'settings'     => array(
            ...
            'options' => array(
                ...
                'add_topic'          => 'Добавление заметки',
                'add_comment'        => 'Добавление комментария',
                'add_blog'           => 'Добавление блога',
                'vote_topic'         => 'Голосование за заметку',
                'vote_comment_topic' => 'Голосование за комментарий к заметке',
    ...
    'feed'          => array(
        ...
        'users' => array(
            'title' => 'Пользователи',
            'note'  => 'Добавьте людей, заметки которых вы хотели бы читать'
        )
    ),
    /**
     * Топик
    ...
    'topic'         => array(
        'topics'       => 'Заметки',
        'topic_plural' => 'заметка;заметки;заметок',
        'drafts'       => 'Черновики',
        ...
        'add'          => array(
            ...
            'notices' => array(
                ...
                'time_limit'             => 'Вам нельзя создавать заметки слишком часто',
        ...
        'blocks'       => array(
            'tip' => array(
                'title' => 'Совет',
                'text'  => '<p><strong>Тег &lt;cut&gt; сокращает длинные записи</strong>, скрывая их целиком или частично под ссылкой («читать дальше»). Скрытая часть не видна в блоге, но доступна в полной записи на странице топика.</p><p><hr/>У нас может писать любой автор. Если нужного Блога нет, обращайтесь к <a href=https://mmotalks.ru/cms/profile/Jolly/>Jolly</a> или <a href=https://mmotalks.ru/cms/profile/XsevenBeta/ >XsevenBeta</a> и мы добавим его.</p><p>Старайтесь придерживаться игровой тематики сайта в своих статьях.<br/>Если нужен совет как оформить статью - прочитайте гайд <a target=_blank href=https://mmotalks.ru/cms/2018/06/28/letsgo.html>Как писать статьи</a></p>',
            )
        )
    ),
    /**
    ...
    'field'         => array(
        'email'       => array(
            'label'   => 'E-mail (письма могут попадать в папку СПАМ)',
            'notices' => array(
    ...
    'rbac'          => array(
        'permission' => array(
            ...
            'vote_topic'               => array(
                ...
                'error' => 'У вас нет прав на голосования за заметки',
            ),
        ),
        'notices'    => array(
